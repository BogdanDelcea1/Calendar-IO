<!-- src/routes/bookings/new/[userId]/+page.svelte -->
...
<script>
  ...
  let users = data.users || []; // List of users to select as participants
  let error = '';
  ...
  async function submitBooking() {
    try {
      ...
      bookingData.participantIds = [...new Set(bookingData.participantIds)];
...
<h1>Create a New Booking for {data.user.username}</h1>
